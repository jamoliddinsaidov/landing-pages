...
const modalMoneyContainers = document.querySelectorAll(
	'.modal-money-container',
);
...
modalOptions.forEach((option, index) =>
	option.addEventListener('click', () => showPrices(index)),
);
...
function showPrices(index) {
	modalMoneyContainers.forEach((container) => {
		if (container.classList.contains('price-show'))
			container.classList.remove('price-show');
	});
